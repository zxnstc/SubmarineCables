        ...
        for point in cable_all_points:
            for land_point_coordinates, land_point_name in list(land_points.items()):
                if str(point) == land_point_coordinates:
                    ...
                    list_node.append([str(point), land_point_name, cable_number, land_point_country])
            ...
            for virtual_point in virtual_points[:]:
                if point == virtual_point:
                    print("yes")
                    str_temp = "vp"+str(x)+","+cable_id
                    list_node.append([str(point), str_temp, 1, 'none'])
                    # csv_writer.writerow([point, "vp"+str(x)+","+cable_id, 1, 'none'])
                    virtual_points.remove(virtual_point)
                    x = x+1

    # # 去重
    df_node = pd.DataFrame(data=list_node)
    df_node.columns = ['coordinates', 'land_name', 'cable_number', 'country']
    print(df_node.shape[0])
    df_node.drop_duplicates(subset=df_node.columns, keep='last', inplace=True)
    print(df_node.shape[0])
    df_node.to_csv(config.l4_node_data_path, encoding='utf-8', index=False)
    ...
    for index1, value1 in enumerate(line_dict["features"]):
        ...
        for line in list_line:
            ...
            for i in range(len(triple_list_name)-1):
                # csv_writer.writerow([triple_list_name[i], triple_list_name[i+1], cable_id, random_num, cable_length,
                #                     cable_owners, cable_suppliers, cable_rfs, cable_rfs_year, cable_is_planned])
                list_edge.append([triple_list_name[i], triple_list_name[i+1], cable_id, random_num, cable_length,
                                  cable_owners, cable_suppliers, cable_rfs, cable_rfs_year, cable_is_planned])

    df_l4_edge = pd.DataFrame(data=list_edge)
    df_l4_edge.columns = ['start_land_name', 'end_land_name', 'cable_id', 'band_width', 'length', 'owners', 'suppliers', 'rfs', 'rfs_year', 'is_planned']
    print(df_l4_edge.shape[0])
    df_l4_edge.drop_duplicates(subset=df_l4_edge.columns, keep='last', inplace=True)
    df_l4_edge = df_l4_edge.loc[~(df_l4_edge['start_land_name'] == df_l4_edge['end_land_name'])]
    print(df_l4_edge.shape[0])
    df_l4_edge.to_csv(config.l4_edge_data_path, encoding='utf-8', index=False)
